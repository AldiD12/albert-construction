...
const trustBadges = [
  { label: "Ltd Company", detail: "Companies House registered" },
  { label: "£10M Insured", detail: "Public liability cover" },
  { label: "2-Year Guarantee", detail: "Written workmanship guarantee" },
  { label: "9.7/10 Checkatrade", detail: "29 verified reviews" },
];
  ...
  return (
    <>
      ...
      {relatedLinks && relatedLinks.length > 0 && (
        <section className="px-6 mb-20 md:mb-28">
          <div className="max-w-7xl mx-auto">
            <h2 className="font-headline text-2xl md:text-3xl font-black text-primary mb-8 tracking-tight text-center">
              Explore Conversion Types
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="group bg-white rounded-lg p-7 shadow-sm border border-outline-variant/20 hover:border-secondary/40 transition-colors flex flex-col"
                >
                  <h3 className="font-headline font-bold text-lg text-primary mb-3 group-hover:text-secondary transition-colors">
                    {link.title} →
                  </h3>
                  <p className="text-charcoal text-sm leading-relaxed flex-1">{link.description}</p>
                </Link>
              ))}
            </div>
          </div>
